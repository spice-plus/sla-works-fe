  ...
  return {
    ...
    mainEntity: {
      ...
      itemListElement: articles.map((article, index) => {
        ...
        const prefecture = company ? getPrefectureByCode(company.prefectureCode) : null
...
export function generatePrefectureSlugStructuredData(
  prefecture: Prefecture,
  slugInfo: { type: 'category'; data: Category } | { type: 'system'; data: SystemName },
  articles: Article[]
) {
  const baseUrl = 'https://sla-works.com'

  const isCategory = slugInfo.type === 'category'
  const name = isCategory ? slugInfo.data.categoryName : slugInfo.data.systemName
  const description = slugInfo.data.description
  const slug = isCategory ? slugInfo.data.categoryNameRoman : slugInfo.data.systemNameRoman
